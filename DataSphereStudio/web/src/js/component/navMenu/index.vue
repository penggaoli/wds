...
export default {
  data() {
    return {
      menuList: [{
        ...
        mainTitle: this.$t('message.headerNavBar.DatalinkDXP'),           // 在下拉框中添加datalink DXP选项
        subTitle: 'datalink dxp',
        children: [{
          classify: this.$t('message.headerNavBar.DatalinkDXP'),
          children: [{
            title: 'Datalink DXP',
            path: 'dxp',
            icon: 'fi-datalink-dxp',
          }]
        }]
      }],
      current: null
  ...
  methods: {
    ...
    handleClick(child) {                                                              // 处理对下拉菜单各项的点击事件
      if (child.path) {
